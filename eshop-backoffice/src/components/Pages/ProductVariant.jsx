...
const TABLE_HEAD_DATA = [
  { id: 1, label: "ID" },
  { id: 2, label: "PRODUCT_ID" },
  { id: 3, label: "COLOR" },
  { id: 4, label: "SIZES" },
  { id: 5, label: "SOLD_COUNT" },
  { id: 6, label: "STOCK" },
];
...
export const ProductVariant = () => {
 const [products, setProducts] = useState([]);
   ...
   useEffect(() => {
     fetch(`${BASE}/product-variants/all`)
       .then(res => res.json())
               ...
               {isResponseMode
                 ...
                 : defaultPaginated.map((product, index) => (
                     <tr key={product.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                       ...
                       <td className="td_item">{product.product?.id}</td>
                       <td className="td_item">{product.color}</td>
                       <td className="td_item">{product.sizes?.join(", ")}</td>
                       <td className="td_item">{product.product.soldCount}</td>
                        <td className="td_item">{product.stock}</td>
                     </tr>
                   ))
               }
             </tbody>
