...
const DressStyle = [
  { id: 9, title: "Casual", img: Img9, flex:2 },
  { id: 10, title: "Formal", img: Img10, flex:3 },
  { id: 11, title: "Party", img: Img11, flex: 3},
  { id: 12, title: "Gym", img: Img12, flex: 2}
]
  ...
  return (
    <div className="browse_style_comp">
      {DressStyle.map((style) => (
        <Link
          key={style.id}
          to={`/dress-style/${style.title.toUpperCase()}`}
          className={`style_card ${
            style.flex === 3 ? "style_card-lg" : "style_card-md"
          }`}
        >
          <img src={style.img} className="style_img" />
          <p className="browse_style_par">{style.title}</p>
        </Link>
      ))}
    </div>
  )
